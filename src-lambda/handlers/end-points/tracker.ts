...
import { trackerSchema } from "../schemas";
...
// Powertools
const TOOL_NAME = `${POWERTOOLS_SERVICE_NAME}-tracker`;
...
export const lambdaHandler = async (
  ...
  try {
    ...
    // ------------------------------------------------------------------------
    const topicArn = process.env.TRACKER_TOPIC_ARN;
    if (!topicArn) {
      throw new Error("TRACKER_TOPIC_ARN is not set");
    }
...
export const handler = middy(lambdaHandler)
  ...
  .use(validator({ eventSchema: transpileSchema(trackerSchema) }))
