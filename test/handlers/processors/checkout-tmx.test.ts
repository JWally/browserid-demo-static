...
// Mock getTmxData
vi.mock("../../../src-lambda/services/get-tmx-data", () => ({
  getTmxData: vi.fn(),
}));
...
describe("checkout-tmx handler", () => {
  ...
  it("propagates error when S3 put fails", async () => {
    ...
    (getTmxData as Mock).mockResolvedValue({ foo: "bar" });
  ...
  it("successfully processes event and sends data to S3", async () => {
    ...
    expect(commandArg.input.Key).toBe(`demo/tmx/${sessionId}.json`);
  ...
  it("processes multiple records", async () => {
    ...
    (getTmxData as Mock).mockImplementation((sessionId: string) =>
      Promise.resolve({ foo: sessionId }),
    );
    const sendSpy = vi
      .spyOn(S3Client.prototype, "send")
      .mockResolvedValue({} as any);
