...
import { handler } from "../../../src-lambda/handlers/processors/tracker-oak";
...
describe("tracker-oak handler", () => {
  ...
  it("propagates error when S3 put fails", async () => {
    ...
    (SQSBodyParser as Mock).mockResolvedValue({
      data: { sessionId: sessionId, other: "value" },
    });
  ...
  it("successfully processes event and sends data to S3", async () => {
    process.env.BUCKET_NAME = "my-bucket";
    const sessionId = "12345";
    const parserData = { data: { sessionId: sessionId, other: "value" } };
    ...
    expect(commandArg.input.Key).toBe(`demo/tracker-oak/${sessionId}.json`);
    ...
    (SQSBodyParser as Mock).mockImplementation((message: string) => {
      if (message === "message1") {
        return Promise.resolve({
          data: { sessionId: sessionId1, other: "value1" },
        });
      } else if (message === "message2") {
        return Promise.resolve({
          data: { sessionId: sessionId2, other: "value2" },
        });
      }
      return Promise.resolve({ data: { sessionId: "unknown" } });
    });
