// test/handlers/end-points/tracker.test.ts
...
//
// ----------------------------------------------------------------------------
describe.sequential("tracker handler", () => {
  beforeEach(() => {
    snsMock.reset(); // reset any previous configurations / calls
    process.env.TRACKER_TOPIC_ARN = "test:topic:arn";
    vi.clearAllMocks();
  ...
  afterEach(() => {
    snsMock.reset();
    vi.resetAllMocks();
    delete process.env.TRACKER_TOPIC_ARN; // Clean up env var
  });
  ...
  test("successfully publishes message to SNS with complete payload", async () => {
    ...
    expect(publishCall.args[0].input).toEqual({
      ...
      Message: expect.stringContaining(SESSION_ID), // Should contain our base64 publicKey
    });
  ...
  //
  // ----------------------------------------------------------------------------
  test("throws error when TRACKER_TOPIC_ARN is not set", async () => {
    delete process.env.TRACKER_TOPIC_ARN;
    snsMock.reset();
    snsMock.on(PublishCommand).resolves({});
    const event = JSON.parse(JSON.stringify(PASSING_EVENT));
    const result = await handler(event as any, MOCK_CONTEXT);
