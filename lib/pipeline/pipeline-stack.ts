...
import { MirvAppStage } from "./pipeline-stages";
...
export class PipelineStack extends Stack {
  constructor(scope: Construct, id: string, props: PipelineStackProps) {
    ...
    const { repo, secretsManager, rootDomain } = props;
    ...
    PIPELINE.forEach((PIPE_STAGE, ndx) => {
      if (PIPE_STAGE.regions.length === 1) {
        const stage = new MirvAppStage(this, PIPE_STAGE.name, rootDomain, {
          env: {
        ...
        PIPE_STAGE.regions.forEach((region) => {
          wave.addStage(
            new MirvAppStage(this, `${PIPE_STAGE.name}-${region}`, rootDomain, {
              env: {
                account: props.env?.account,
                region,
              },
